...
for(const item of products){const mapped=imageRegistry[item.id];item.slug=slugRegistry[item.id];item.images=Array.isArray(mapped)?mapped:mapped?[mapped]:[];item.image=item.images[0]||null;if(!item.slug)throw new Error(`Missing English slug for ${item.id}`);}
...
for(const item of products){
  ...
  const gallery=item.images.length?item.images.map(image=>`<img class="product-image" src="../../${esc(image)}" alt="${esc(item.name)}" decoding="async">`).join(""):`<i class="bi ${esc(item.icon)}" aria-hidden="true"></i>`;
