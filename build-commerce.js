...
function card(item,prefix=""){
  ...
  return `<article class="ecom-card" data-product-card data-category="${esc(item.category)}" data-search="${search}"><a class="ecom-visual" href="${prefix}shop/product/${item.id}.html" aria-label="View ${esc(item.name)}"><span class="ecom-badge">${esc(item.categoryName)}</span><i class="bi ${esc(item.icon)}" aria-hidden="true"></i></a><div class="ecom-card-body"><p class="ecom-category">${esc(item.categoryName)}</p><h3><a href="${prefix}shop/product/${item.id}.html">${esc(item.name)}</a></h3><p class="ecom-pack">${esc(sizes)}</p><p class="ecom-price">${variant?money(variant.price):"Currently unavailable"}</p><div class="ecom-actions"><a class="btn btn-outline-primary rounded-pill" href="${prefix}shop/product/${item.id}.html">View</a><button class="btn btn-primary btn-icon rounded-circle" type="button" data-add-to-cart data-product-id="${item.id}" data-variant-index="${Math.max(index,0)}" ${variant?"":"disabled"} aria-label="Add ${esc(item.name)} to cart"><i class="bi bi-bag-plus"></i></button></div></div></article>`;
}
...
for(const category of catalog.categories){
  ...
  html=replaceRegion(html,"category-products",section);html=addCatalogScript(html,"../../");fs.writeFileSync(file,html,"utf8");
}
