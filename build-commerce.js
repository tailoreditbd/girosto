...
for(const item of products){item.slug=slugRegistry[item.id];item.image=imageRegistry[item.id]||null;if(!item.slug)throw new Error(`Missing English slug for ${item.id}`);}
...
function card(item,prefix=""){
  ...
  const visual=item.image?`<img class="ecom-image" src="${prefix}${esc(item.image)}" alt="${esc(item.name)}" loading="lazy" decoding="async">`:`<i class="bi ${esc(item.icon)}" aria-hidden="true"></i>`;
...
for(const category of catalog.categories){
  ...
  html=placeAfterPageHero(html,"category-products",section);html=addCatalogScript(html,"../../");fs.writeFileSync(file,html,"utf8");
}
